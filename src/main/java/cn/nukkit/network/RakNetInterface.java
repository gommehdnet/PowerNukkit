...
public class RakNetInterface implements RakNetServerListener, AdvancedSourceInterface {
    ...
    public boolean process() {
        ...
        while ((session = this.sessionCreationQueue.poll()) != null) {
            ...
            try {
                ...
                Constructor<? extends Player> constructor = event.getPlayerClass().getConstructor(SourceInterface.class, Long.class, InetSocketAddress.class, int.class);
                Player player = constructor.newInstance(this, event.getClientId(), event.getSocketAddress(), this.raknet.getRakNetVersionByAddress(address));
    ...
    public void onSessionCreation0(RakNetServerSession session) {
        final RakNetPlayerSession rakNetPlayerSession = new RakNetPlayerSession(this, session);

        session.setListener(rakNetPlayerSession);
    ...
    public Integer putResourcePacket(Player player, DataPacket packet) {
        ...
        if (session != null) {
            ...
            packet.tryEncode();
            session.sendPacket(packet.clone());
        }
