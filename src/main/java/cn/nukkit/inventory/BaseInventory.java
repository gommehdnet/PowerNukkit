...
public abstract class BaseInventory implements Inventory {
    ...
    public Item getItem(int index) {
        return this.slots.containsKey(index) ? this.slots.get(index).clone() : Item.get(ItemID.AIR);
    }
    ...
    public boolean setItem(int index, Item item, boolean send) {
        ...
        if (index < 0 || index >= this.size) {
            return false;
        } else if (item.getIdentifier().equals(ItemID.AIR) || item.getCount() <= 0) {
    ...
    public int firstEmpty(Item item) {
        for (int i = 0; i < this.size; ++i) {
            if (this.getItem(i).getIdentifier() == ItemID.AIR) {
                return i;
            }
        }
    ...
    public boolean canAddItem(Item item) {
        ...
        for (int i = 0; i < this.getSize(); ++i) {
            ...
            } else if (slot.getIdentifier() == ItemID.AIR) {
    ...
    public Item[] addItem(Item... slots) {
        ...
        for (Item slot : slots) {
            if (slot.getIdentifier() != ItemID.AIR && slot.getCount() > 0) {
    ...
    public void onSlotChange(int index, Item before, boolean send) {
        ...
        if (before.getIdentifier() == ItemID.LODESTONE_COMPASS || getItem(index).getIdentifier() == ItemID.LODESTONE_COMPASS) {
    ...
    public boolean isFull() {
        ...
        for (Item item : this.slots.values()) {
            if (item == null || item.getIdentifier() == ItemID.AIR || item.getCount() < item.getMaxStackSize() || item.getCount() < this.getMaxStackSize()) {
                return false;
            }
        }
    ...
    public int getFreeSpace(Item item) {
        ...
        for (Item slot : this.getContents().values()) {
            if (slot == null || slot.getIdentifier() == ItemID.AIR) {
                space += maxStackSize;
                continue;
            }
