...
public class GrindstoneInventory extends FakeBlockUIComponent {
    ...
    public boolean updateResult(boolean send) {
        ...
        if (!firstItem.isNull() && !secondItem.isNull() && firstItem.getIdentifier() != secondItem.getIdentifier()) {
            setResult(Item.get(ItemID.AIR), send);
        ...
        if (firstItem.getIdentifier() == ItemID.ENCHANTED_BOOK) {
