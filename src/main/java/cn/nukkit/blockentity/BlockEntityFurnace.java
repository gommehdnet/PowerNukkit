...
public class BlockEntityFurnace extends BlockEntitySpawnable implements InventoryHolder, BlockEntityContainer, BlockEntityNameable {
    ...
    protected void checkFuel(Item fuel) {
        ...
        if (burnTime > 0 && ev.isBurning()) {
            ...
            if (fuel.getCount() == 0) {
                if (fuel.getIdentifier() == ItemID.BUCKET && ((ItemBucket)fuel).isLava()) {
                    fuel.setDamage(0);
                    fuel.setCount(1);
                } else {
                    fuel = Item.get(ItemID.AIR);
                }
            }
    ...
    public boolean onUpdate() {
        ...
        boolean canSmelt = (smelt != null && raw.getCount() > 0 && ((smelt.getResult().equals(product, true) && product.getCount() < product.getMaxStackSize()) || product.getIdentifier() == ItemID.AIR));
        ...
        if (burnTime > 0) {
            ...
            if (smelt != null && canSmelt) {
                cookTime++;
                if (cookTime >= readyAt) {
                    product = Item.get(smelt.getResult().getIdentifier(), 0, product.getCount() + 1);
