...
public class BlockExposedCutCopperSlab extends BlockSlab {
    ...
    public BlockExposedCutCopperSlab(int meta) {
        super(meta, BlockID.EXPOSED_DOUBLE_CUT_COPPER_SLAB);
    }
    ...
    @Override
    public Item toItem() {
        return Item.get(ItemID.EXPOSED_CUT_COPPER_SLAB);
    }
    ...
    public boolean onActivate(@Nonnull Item item, @Nullable Player player) {
        if (item.getIdentifier().equals(ItemID.HONEYCOMB)) {
            this.getLevel().setBlock(this, Block.get(BlockID.WAXED_EXPOSED_CUT_COPPER_SLAB), true, true);
        ...
        if (item.isAxe()) {
            this.getLevel().setBlock(this, Block.get(BlockID.CUT_COPPER_SLAB), true, true);
