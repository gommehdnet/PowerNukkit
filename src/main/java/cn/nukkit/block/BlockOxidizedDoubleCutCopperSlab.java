...
public class BlockOxidizedDoubleCutCopperSlab extends BlockDoubleSlabBase {
    ...
    public boolean onActivate(@Nonnull Item item, @Nullable Player player) {
        if (item.getIdentifier().equals(ItemID.HONEYCOMB)) {
            this.getLevel().setBlock(this, Block.get(BlockID.WAXED_OXIDIZED_DOUBLE_CUT_COPPER_SLAB), true, true);
        ...
        if (item.isAxe()) {
            this.getLevel().setBlock(this, Block.get(BlockID.WEATHERED_DOUBLE_CUT_COPPER_SLAB), true, true);
