...
public class BlockFrame extends BlockTransparentMeta implements BlockEntityHolder<BlockEntityItemFrame>, Faceable {
    @PowerNukkitOnly
    @Since("1.4.0.0-PN")
    ...
    public BlockFrame(int meta) {
        super(meta);
    }

    @Override
    public BlockID getId() {
        return BlockID.FRAME;
    }
    ...
    public int onUpdate(int type) {
        if (type == Level.BLOCK_UPDATE_NORMAL) {
            ...
            if (!support.isSolid() && support.getId() != BlockID.COBBLESTONE_WALL) {
    ...
    public boolean onActivate(@Nonnull Item item, Player player) {
        ...
        if (itemFrame.getItem().isNull()) {
            ...
            if (itemOnFrame.getIdentifier() == ItemID.FILLED_MAP) {
                setStoringMap(true);
    ...
    public boolean place(@Nonnull Item item, @Nonnull Block block, @Nonnull Block target, @Nonnull BlockFace face, double fx, double fy, double fz, @Nullable Player player) {
        if ((!(target.isSolid() || target instanceof BlockCobblestoneWall)  && !target.equals(block) || (block.isSolid() && !block.canBeReplaced()))) {
            return false;
        }
        ...
        if (target.equals(block) && block.canBeReplaced()) {
            face = BlockFace.UP;
            target = block.down();
            if (!target.isSolid() && !(target instanceof BlockCobblestoneWall)) {
                return false;
            }
        }

        setBlockFace(face);
        setStoringMap(item.getIdentifier() == ItemID.FILLED_MAP);
