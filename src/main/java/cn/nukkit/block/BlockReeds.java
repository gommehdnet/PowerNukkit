...
public class BlockReeds extends BlockFlowable {
        ...
        if (item.isFertilizer()) { //Bonemeal
            ...
            if (count < 3) {
                ...
                for (int i = 1; i <= toGrow; i++) {
                    ...
                    } else if (block.getId() != BlockID.REEDS) {
                        break;
                    }
                }
    ...
    public int onUpdate(int type) {
        ...
        if (type == Level.BLOCK_UPDATE_RANDOM) {
            ...
            int height = 0;
            for (Block current = this; height < 3 && current.getId() == BlockID.REEDS; height++) {
    ...
    private boolean isSupportValid() {
        ...
        if (downId != BlockID.GRASS && downId != BlockID.DIRT && downId != BlockID.SAND || down.getId() == BlockID.PODZOL) {
            return false;
        }
