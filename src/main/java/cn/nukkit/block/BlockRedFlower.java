 ...
 * @since 2015/11/23
 */
public class BlockRedFlower extends BlockFlowable {
    @PowerNukkitOnly
    @Since("1.4.0.0-PN")
    ...
    public BlockRedFlower(int meta) {
        super(meta);
    }
    ...
    public boolean onActivate(@Nonnull Item item, Player player) {
        if (item.isFertilizer()) { //Bone meal
            ...
            for (int i = 0; i < 8; i++) {
                ...
                if (level.getBlock(vec).getId() == BlockID.AIR && level.getBlock(vec.down()).getId() == BlockID.GRASS && vec.getY() >= 0 && vec.getY() < 256) {
