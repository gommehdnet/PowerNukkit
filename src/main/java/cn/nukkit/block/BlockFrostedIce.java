...
@PowerNukkitOnly
public class BlockFrostedIce extends BlockTransparentMeta {
    ...
    @PowerNukkitOnly
    public BlockFrostedIce(int meta) {
        super(meta);
    }

    @Override
    public BlockID getId() {
        return BlockID.FROSTED_ICE;
    }
    ...
    public boolean onBreak(Item item) {
        level.setBlock(this, get(BlockID.FLOWING_WATER), true);
        return true;
    }
    ...
    public int onUpdate(int type) {
        if (type == Level.BLOCK_UPDATE_SCHEDULED) {
            ...
            if (countNeighbors() < 2) {
                level.setBlock(this, layer, get(BlockID.FLOWING_WATER), true);
            }
        }
    ...
    @Override
    public Item toItem() {
        return Item.get(ItemID.FROSTED_ICE);
    }
            ...
            if (isSource) {
                for (BlockFace face : BlockFace.values()) {
                    ...
                    if (block instanceof BlockFrostedIce) {
                        ((BlockFrostedIce) block).slightlyMelt(false);
                    }
                }
            }
        }
    ...
    private int countNeighbors() {
        int neighbors = 0;
        for (BlockFace face : BlockFace.values()) {
            if (getSide(face).getId() == BlockID.FROSTED_ICE && ++neighbors >= 4) {
                return neighbors;
            }
        }
        return neighbors;
