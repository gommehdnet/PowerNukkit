...
public class BlockWeatheredCutCopperStairs extends BlockStairs {
    ...
    public boolean onActivate(@Nonnull Item item, @Nullable Player player) {
        if (item.getIdentifier().equals(ItemID.HONEYCOMB)) {
            this.getLevel().setBlock(this, Block.get(BlockID.WAXED_WEATHERED_CUT_COPPER_STAIRS), true, true);
        ...
        if (item.isAxe()) {
            this.getLevel().setBlock(this, Block.get(BlockID.EXPOSED_CUT_COPPER_STAIRS), true, true);
