    ...
    public boolean generate(ChunkManager worldIn, NukkitRandom rand, Vector3 vectorPosition) {
        ...
        if (position.getY() >= 1 && position.getY() + i + 1 <= 256) {
            ...
            if (!flag) {
                ...
                BlockID block = worldIn.getBlockIdAt(down.x, down.y, down.z);
                ...
                if ((block == BlockID.GRASS || block == BlockID.DIRT) && position.getY() < 256 - i - 1) {
                        ...
                        for (int j3 = position.getX() - l2; j3 <= position.getX() + l2; ++j3) {
                            ...
                            for (int i4 = position.getZ() - l2; i4 <= position.getZ() + l2; ++i4) {
                                ...
                                if (Math.abs(k3) != l2 || Math.abs(j1) != l2 || rand.nextBoundedInt(2) != 0 && j2 != 0) {
                                    ...
                                    BlockID id = worldIn.getBlockIdAt(blockpos.x, blockpos.y, blockpos.z);
                    ...
                    for (int l1 = 0; l1 < i; ++l1) {
                        ...
                        BlockID id = worldIn.getBlockIdAt(up.x, up.y, up.z);

                        if (id == BlockID.AIR || id == BlockID.LEAVES || id == BlockID.FLOWING_WATER || id == BlockID.WATER) {
